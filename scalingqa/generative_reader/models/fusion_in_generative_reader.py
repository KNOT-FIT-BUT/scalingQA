# This implementation is based upon the implementation of 'T5ForConditionalGeneration' class from 'transformers' library.
# The original implementation is distributed under Apache 2.0 license and released by TensorFlow authors, T5 Authors and HuggingFace Inc. team.
...
class T5FusionInDecoder(T5PreTrainedModel):
    ...
    def from_pretrained(clz, config, do_not_download_weights=False, **kwargs):
        cfg = T5Config.from_pretrained(config['reader_transformer_type'], cache_dir=config["transformers_cache"])
        ...
        return super(T5FusionInDecoder, clz).from_pretrained(
            config['reader_transformer_type'], config=cfg, cache_dir=config["transformers_cache"], **kwargs)
