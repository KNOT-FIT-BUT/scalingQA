...
from ..models.reader_without_joint import ReaderWithoutJoint
...
class TrainerWithoutJoint(object):
    """
    ...
    def init_model(self) -> torch.nn.Module:
        """
        Initialization of extractive_reader model.

        :return: Initialized model.
        :rtype: torch.nn.Module
        """

        model = ReaderWithoutJoint(self.config)
    ...
    @staticmethod
    def _useModel(model: ReaderWithoutJoint, batch: ReaderBatch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Uses model on given batch.

        :param model: The extractive_reader model.
        :type model: Reader
    ...
    @torch.no_grad()
    def validate(self, model: ReaderWithoutJoint, val: DataLoader, log_results: bool = True) -> Tuple[
        ...
        for i, batch in iterator:
            ...
            startScores, endScores, selectionScore = self._useModel(model, batchOnDevice)

            logProbs = ReaderWithoutJoint.scores2logSpanProb(startScores, endScores, selectionScore)
            ...
            if self.config["get_answer_mask_for_validation"]:
                if batchOnDevice.answersMask.any():
                    ...
                    loss = ReaderWithoutJoint.marginalCompoundLoss(logProbs, batchOnDevice.answersMask)
                    loss_sum += loss.item()
                    samplesWithLoss += 1
    ...
    def train_epoch(self, model: ReaderWithoutJoint, optimizer: torch.optim.Optimizer, scaler: GradScaler,
                    train: DataLoader,
            ...
            try:
                with torch.cuda.amp.autocast(enabled=self.config["mixed_precision"]):
                    ...
                    if not self.config["independent_components_in_loss"]:
                        ...
                        logSpanProb = ReaderWithoutJoint.scores2logSpanProb(startScores, endScores, selectionScore)
                    ...
                    if self.config["hard_em_steps"] > 0 and \
                            random.random() <= min(self.update_it / self.config["hard_em_steps"],
                                                   self.config["max_hard_em_prob"]):
                        # loss is calculated for the max answer span with max probability
                        if self.config["independent_components_in_loss"]:
                            loss = ReaderWithoutJoint.hardEMIndependentComponentsLoss(startScores, endScores,
                                                                                      selectionScore,
                                                                                      batchOnDevice.answersMask)
                        else:
                            loss = ReaderWithoutJoint.hardEMLoss(logSpanProb, batchOnDevice.answersMask)
                    else:
                        # loss is calculated for all answer spans
                        if self.config["independent_components_in_loss"]:
                            loss = ReaderWithoutJoint.marginalCompoundLossWithIndependentComponents(startScores,
                                                                                                    endScores,
                                                                                                    selectionScore,
                                                                                                    batchOnDevice.answersMask)
                        else:
                            loss = ReaderWithoutJoint.marginalCompoundLoss(logSpanProb, batchOnDevice.answersMask)
